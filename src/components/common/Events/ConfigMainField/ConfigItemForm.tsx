...
const ConfigItemForm = ({ control, index, errors, templateConfigFields, isMainField = false, onRemove }: Props) => {
  // ** I18n
  ...
  const renderConfigFields = () => {
    ...
    for (let i = 0; i < desktop.length; i++) {
      ...
      const labels = Object.keys(templateConfigFields?.attributes?.desktop).map(
        (key) => templateConfigFields?.attributes?.desktop?.[key]?.label,
      );

      configFieldsEls.push(
        <div key={`configFields${i}`} className="grid grid-cols-4 gap-x-3 md:grid-cols-7 mb-3 last:mb-0">
          <div className="h-12 flex items-center">{labels?.[i]}</div>
          {desktopFieldEl}
          {tabletFieldEl}
          {mobileFieldEl}
        </div>,
  ...
  return (
    <div>
      <div className="grid grid-cols-12 gap-3 xl:flex">
        ...
        <div
          className={clsx(
            "col-span-2 md:col-span-1 xl:w-20",
        ...
        <div className="self-center col-span-2 md:hidden">
          <ConfirmPopover
            ...
            mainTitle={translation("deletePopover.titleEventConfigField")}
        ...
        <div
          className={clsx(
            "col-span-6 md:col-span-4 xl:flex-1",
        ...
        <div className={clsx("col-span-6 md:col-span-5 xl:flex-[2]")}>
          <Controller
            control={control}
        ...
        {/* Action Button */}
        <div className={clsx("hidden md:col-span-2 md:flex md:items-center md:h-12 xl:w-fit")}>
          <IconButton sx={{ color: themeColors.colors.blue219 }}>
            <SaveIcon fontSize="small" />
          </IconButton>
        ...
        <div className="mt-5">
          ...
          <div>
            <div className="grid items-center grid-cols-4 gap-x-3 md:grid-cols-7 mb-3">
              ...
              <div className="font-semibold md:col-span-2">{translation("label.tablet")}</div>
