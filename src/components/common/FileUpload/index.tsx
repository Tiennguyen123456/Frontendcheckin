...
import { MAX_FILE_SIZE } from "@/constants/variables";
import { toastError } from "@/utils/toast";
import ImageUpload from "./ImageUpload";
import { cn, validateFileType } from "@/utils/common";
    ...
    switch (action.type) {
      case "ADD_FILES_TO_INPUT": {
        ...
        if (state.length + action.payload.length > 5) {
          toastError("Too many files. You can only upload a maximum of 5 files at a time.");
          return state;
        }
  ...
  const handleChange = async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      if (e.target.files && e.target.files[0]) {
        ...
        if (!valid) {
          toastError("Invalid file type. Please upload a valid file type.");
          return;
        }
  ...
  const handleDrop = async (e: DragEvent<HTMLDivElement>) => {
    ...
    if (e.dataTransfer.files && e.dataTransfer.files[0]) {
      ...
      if (files.length !== validFiles.length) {
        toastError("Invalid file type. Only image files are allowed.");
      }
              ...
              <div className="overflow-x-auto sm:-mx-6 lg:-mx-8">
                <div className="align-middle inline-block min-w-full sm:px-6 lg:px-8">
                  <div className="shadow overflow-hidden sm:rounded-lg">
                    ...
                    >
                      <AddIcon className="group-hover:fill-slate-400 transition stroke-1 w-12 h-12 fill-slate-500" />
                      <input
                        {...props}
                        ref={ref}
                        multiple
